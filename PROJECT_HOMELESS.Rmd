...
```
```{r}
#homeless_data <- homlessness_data_2014_2017 %>%
#  filter(year==2015)
```
```{r}
homlessness_data_2014_2017 %>%
  sum(homeless_individuals)
```
...
homelessness <- homlessness_data_2014_2017 %>%
  select(name, total_homeless,unsheltered_homeless, unemployment, price, population, programs_funded)

```
Creating a preditive model :
Q1. Is unemployment directly or indectly affecting the homelessness in USA?
```{r}
model_1 = lm(total_homeless ~ programs_funded, homelessness)
```
```{r}
...
```
```{r}
ggplot(homelessness, aes(x= programs_funded, y= total_homeless))+
  geom_smooth(method="lm", se=FALSE)+
  geom_segment(aes(xend= programs_funded, yend= predicted), alpha=.3)+
  geom_point(aes(color= abs(residuals), size= abs(residuals)))+
  scale_color_continuous(low="green", high="red")+
  theme_bw()+
  plot(model_1) 
 # the above function plot() helps us to view the data in differnt graphs 
```
MODEL_2
Q2. As the rent prices have always been increasing this pridiction will tell us if rent prices are the cause of unsheltered homelessness.
...
Q3 Does unemployment contribute to homelessness among families?
```{r}
homelessness <- homlessness_data_2014_2017 %>%
